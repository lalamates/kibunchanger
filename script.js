...
document.getElementById('form').onsubmit = function (event) {
    ...
    const fukurofamous = ['五輪書', '論語' , '武士道' , '孫子' , ' 韓非子' , '菜根譚' , '風姿花伝' , '老子' , '呉子' , '荘子' , '貞観政要' , '' ];
    const ffr = fukurofamous[search4];

    const shikafamous = ['愛染恭子', '逢初夢子', '青木鶴子', '赤坂小梅', 'あき竹城', '麻丘めぐみ', '朝丘雪路', '浅丘ルリ子', '朝加真由美', '浅香光代', '浅田美代子', '朝比奈順子', '梓英子', '東龍子', '東屋トン子', 'あべ静江', '天地総子', '有馬稲子', '淡路恵子', '淡島千景', '天地真理'];
    const sfr = shikafamous[search5];

    const shikafamous2 = ['ダイナクロコvsスーパーゲイター', 'ダイナソーinL.A.', 'クラーケンフィールド/HAKAISHIN', 'ダイ・ハード2', 'メガ・シャークVSジャイアント・オクトパス', 'ピラニア3D', 'エイリアン vs.アバター', 'シャークネード', 'マンイーター', 'ディノシャーク', 'マンモス', 'ビーチ・シャーク', 'ヴァイキングダム', 'ツイスター2008', 'ボトム・プレデター 地底に潜む生命体', 'アイス・ツイスター', 'アポカリプス・ナウ', 'ビッグ・バグズ・パニック', 'ヒート・ストローク', 'トランスバトラー', '2048'];
    const sfr2 = shikafamous2[search6];
    
    const shikafamous3 = ['ドラえもん', '安倍元首相', 'しずかちゃん', '虎のじゅうたん', 'スネ夫', 'こなきじじい', 'ジョージ・クルーニー', 'カオナシ', 'バイデン大統領', 'twitterのロゴ', 'ムック', 'ねずみ男', 'マリオ', 'ピカチュウ', 'フリーザ', '波平', 'ねこバス', ];
    const sfr3 = shikafamous3[search7];
    
    const shikafamous4 = ['著作者、手術中', '赤巻き紙、青巻き紙、黄巻き紙', '坊主が上手に屏風に坊主の絵をかいた', '除雪車、除雪作業中', '隣の客はよく柿食う客だ', '赤パジャマ、黄パジャマ、青パジャマ', '生麦 生米 生卵', '東京特許許可局', '老若男女', '親切診察室視察', '肩固かったから買った肩たたき器', '骨粗鬆症訴訟勝訴', 'バナナの謎はまだ謎なのだぞ', '空虚な九州空港の究極高級航空機', 'マグマ大使のママはマママグマ大使', '庭には二羽鶏がいる', '信長どのも信長どのなら、ねねどのもねねどのじゃ', ];
    const sfr4 = shikafamous4[search8];
    
    const shikafamous5 = ['神の手を持つ外科医','世間話が上手い美容師', 'テキトーに弁護する弁護士', '潔癖症な検視官', 'カット失敗されて美容師に文句言う客', '信号無視で止められたけど逆ギレするドライバー', '1分で1億売るTV通販カリス実演販売士', 'ペンタゴンのデータにまで侵入する天才ハッカー', '財界からも信頼度の高いスーパー占い師', '台風の状況を外から報告するリポーター', '最近買ったコスメを紹介するカリスマYoutuber', 'しゃべれるようになったお嬢様猫', '庭から居住者をじっと見つめる木', '隣の隣の奥さんの噂話をしてくる隣の奥さん', '午後2時ごろから酔っぱらうおじさん', '来日して日本のファンに日本語で語りかけるハリウッドスター','無銭飲食の謝罪会見をする力士'];
    const sfr5 = shikafamous5[search8];
    
    const shikafamous6 = ['桜','寿', '凛と咲く', '龍', '秋の旅人', '流氷', '宇宙', '未来', '森は知っている', 'カサブランカ', '極み', '滝', '爆発', '怒り', '歓喜', '熱狂','古(いにしえ)'];
    const sfr6 = shikafamous6[search9];
    
    
    
const fukurosay = ['人生は短い。今一度、挑戦してみたいことを1つ紙に書いてくださらんか。',
    ...
    const shikasay = ['ものまねできそうな人の曲をあげて、本人のつもりで1曲歌いあげてくださいますか？',
...
if(search>20 || search<1) {
            ...
            // アニメーションさせたいクラス
            let container01 = $(".serihuTxt01");
            // アニメーションスピード
            let speed = 80;
            
            // テキストの間にスペースを入れます
            let content01 = $(container01).html();
            let text01 = $.trim(content01);
            let newHtml01 = "";
            
            // スペースで区切ったテキストを、テキストの数だけspanで囲む
            text01.split("").forEach(function(v1) {
             newHtml01 += '<span>' + v1 + '</span>';
            });
            
            // spanで囲んだテキスト群をHTMLに戻す
            $(container01).html(newHtml01);
            
            // 1文字ずつ表示
            let txtNum01 = 0;
            setInterval(function() {
              $(container01).find('span').eq(txtNum01).css({opacity: 1});
             txtNum01++
            }, speed);
            ...
            // アニメーションさせたいクラス
let container02 = $(".serihuTxt02");
// アニメーションスピード
 speed = 80;

// テキストの間にスペースを入れます
let content02 = $(container02).html();
let text02 = $.trim(content02);
let newHtml02 = "";

// スペースで区切ったテキストを、テキストの数だけspanで囲む
text02.split("").forEach(function(v2) {
 newHtml02 += '<span>' + v2 + '</span>';
});
...
let container03 = $(".serihuTxt03");
// アニメーションスピード


// テキストの間にスペースを入れます
let content03 = $(container03).html();
let text03 = $.trim(content03);
let newHtml03 = "";

// スペースで区切ったテキストを、テキストの数だけspanで囲む
text03.split("").forEach(function(v3) {
 newHtml03 += '<span>' + v3 + '</span>';
});

// spanで囲んだテキスト群をHTMLに戻す
$(container03).html(newHtml03);

// 1文字ずつ表示
let txtNum03 = 0;
setInterval(function() {
  $(container03).find('span').eq(txtNum03).css({opacity: 1});
 txtNum03++
}, speed);
